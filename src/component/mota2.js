...
const ProductDetails = () => {
  const producs = useSelector((state) => state.producs);

  const { id, trademark, image, price1, price, name, machie } = producs;
  const { productIds } = useParams();

  const dispatch = useDispatch();

  const callApis = async (id) => {
    const responsess = await axios
      .get(`https://607ab7e2bd56a60017ba2fe2.mockapi.io/api/sicle/${id}`)
      .catch((err) => {
        console.log(err);
      });
    dispatch(selectedProducts(responsess.data));
  };
  useEffect(() => {
    if (productIds && productIds !== "") callApis(productIds);
  }, [productIds]);

  return (
    <div className="ui grid container">
      {Object.keys(producs).length === 0 ? (
        <div>...Loading</div>
      ) : (
        <div>
          <div className="title-Motas">
            <span>Trang chủ / {name}</span>
          </div>
          <div className="container__Mota">
            <div className="containet_fulter">
              <div className="container_fulter1">
                <img className="image-Mota" src={image} />
              </div>
                  ...
                  <div className="description1">
                    <div>
                      <h3 className="price-des">
                        <strong className="price-des">{price}</strong>
                      </h3>
                      <h5 className="price-dess">
                        <strong className="price-dess">{price1}</strong>
                      </h5>
                    </div>
                    <div className="information-des">
                      <div>
                        <span>
                          <span className="informas">Giới tính :</span>Unisex
                        </span>
                      </div>
                      <br></br>
                      <div className="informa">
                        <span>
                          <span className="informas">Màu sắc :</span>
                          {machie}
                        </span>
                      </div>
                      <br />
                      <div className="informa">
                        <span>
                          <span className="informas">Xuất xứ :</span> Anh
                        </span>
                      </div>
                      <br />
                      <div className="informa">
                    ...
                    <div className="share">
                      <span>
                        CHIA SẺ QUA <AiFillFacebook className="share-icon" />{" "}
                  ...
                  <div className="description2">
                    <div className="dnescriptio2-2">
                      ...
                      <div className="description2-2_3">
                        ...
                        />
                        <span>
                          miễn phí trả đổi trong 7 ngày <Link>chi tiết</Link>{" "}
                        </span>
                        <br />
                        <img
                          src="https://vuahanghieu.com/assets/images/checkbox.svg"
                        ...
                        <br />
                        <p className="contrac">
                          bạn muốn bán hàng cùng vua hãy ? <Link>đăng kí</Link>
                        </p>
                      </div>
                    </div>
                  </div>
